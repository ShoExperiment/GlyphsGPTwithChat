#MenuTitle: GlyphsGPT with Chat for Default Python
# -*- coding: utf-8 -*-
__doc__="""
GlyphsGPT with Chat for Default Python
"""
...
def http_post_json(url, payload, headers=None, timeout=25):
    ...
    # Prefer Apple TLS for HTTPS (uses macOS trust, fixes CERTIFICATE_VERIFY_FAILED)
    if url.lower().startswith("https") and HAS_NSURLSESSION and PREFER_APPLE_TLS:
    ...
    try:
        ...
        s = str(e)
        # If urllib+ssl barfs on certs, retry via NSURLSession
        if url.lower().startswith("https") and HAS_NSURLSESSION and (
...
def http_get_json(url, headers=None, timeout=10):
    ...
    try:
        ...
        s = str(e)
        # Auto-fallback to NSURLSession on cert issues
        if url.lower().startswith("https") and HAS_NSURLSESSION and (
            "CERTIFICATE_VERIFY_FAILED" in s or "ssl" in s.lower()
        ):
            return _ns_request_json("GET", url, None, headers, timeout)
...
class HTMLChatUI(object):
    ...
    def _post_chat_with_compat(self, url, headers, payload_base, max_out):
        ...
        - 'max_tokens' → 'max_completion_tokens' when rejected
        - drop temperature/top_p if the server forbids them
        - hint/adjust if the host expects Responses-style knobs
        - backoff on timeouts
        - handle BOTH HTTP errors AND JSON {"error":...} bodies
        """
        pay = dict(payload_base)
        pay["max_tokens"] = int(max_out)
    
        # 20s base + ~40ms per requested token, clamped 35..300s
        timeout_s = max(35, min(300, 20 + int(int(max_out) * 0.04)))
    
        def _maybe_retry_from_json_error(res):
            # returns True if we mutated 'pay' and should retry
            if not (isinstance(res, dict) and isinstance(res.get("error"), dict)):
                return False
            err = res["error"]
            msg = str(err.get("message", "")) or str(res)
            param = str(err.get("param", ""))
    
            # 1) max_tokens unsupported → use max_completion_tokens
            if ("Unsupported parameter" in msg and "max_tokens" in msg) or param == "max_tokens":
                pay.pop("max_tokens", None)
                pay["max_completion_tokens"] = int(max_out)
                return True
    
            # 2) Responses-style hint (some proxies mention 'max_output_tokens' or 'responses' API)
            if ("max_output_tokens" in msg) or ("responses" in msg.lower()):
                pay.pop("max_tokens", None)
                pay["max_completion_tokens"] = int(max_out)
                return True
    
            # 3) temperature/top_p sometimes disallowed
            if "temperature" in msg and "Unsupported" in msg and "temperature" in pay:
                pay.pop("temperature", None)
                return True
            if "top_p" in msg and "Unsupported" in msg and "top_p" in pay:
                pay.pop("top_p", None)
                return True
        ...
        while attempts < 6:
            attempts += 1
            try:
                res = http_post_json(url, pay, headers=headers, timeout=timeout_s)
    
                # If the server returned a JSON error with HTTP 200, try to adapt and retry.
                if _maybe_retry_from_json_error(res):
                    continue
    
                return res  # success or a non-adaptable error JSON that caller will surface
    
            except RuntimeError as e:
                s = str(e)
                last_err = e
    
                # Back off on timeouts
                if ("timed out" in s.lower()) or ("timeout" in s.lower()) or ("deadline" in s.lower()):
                    timeout_s = min(300, int(timeout_s * 1.6) + 5)
                    continue
